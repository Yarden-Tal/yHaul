var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
confirm('Dear admin, please note that y-Haul must approve of any changes to prices and/or products.');
function renderTrucksAsAdmin() {
    return __awaiter(this, void 0, void 0, function () {
        var trucksToRender, root, html_1, trucks, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, axios.get("/trucks")];
                case 1:
                    trucksToRender = _a.sent();
                    root = document.querySelector('.trucks-wrapper');
                    html_1 = "";
                    trucks = trucksToRender.data;
                    console.log(trucks);
                    trucks.forEach(function (truck) {
                        html_1 += "<div class=\"truck\">\n            <div class=\"truck__img\">\n                <img src=\"" + truck.image + "\" alt=\"" + truck.type + "\">\n            </div>\n            <div class=\"truck__type\">" + truck.type + "</div>\n            <p class=\"truck__price\">" + truck.price + "$</p>\n            <i class=\"fas fa-edit truck__edit\" onclick=\"handleEdit('" + truck.id + "')\"></i>\n            <i class=\"fas fa-trash truck__delete\" onclick=\"handleDelete('" + truck.id + "')\"></i>\n        </div>";
                    });
                    root.innerHTML = html_1;
                    return [3 /*break*/, 3];
                case 2:
                    error_1 = _a.sent();
                    console.error(error_1);
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    });
}
renderTrucksAsAdmin();
// Add
var handleAdd = function (ev) { return __awaiter(_this, void 0, void 0, function () {
    var name, price, imgUrl, newTruck, request, er_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                ev.preventDefault();
                name = ev.target.elements.name.value;
                price = ev.target.elements.price.value;
                imgUrl = ev.target.elements.img.value;
                console.log('form input:', name, price, imgUrl);
                if (!name || !price || !imgUrl) {
                    throw new Error("One or more fields are missing");
                }
                ev.target.reset();
                newTruck = { type: name, price: price, image: imgUrl };
                console.log('newTruck:', newTruck);
                return [4 /*yield*/, axios.post('/trucks', { type: name, price: price, image: imgUrl })];
            case 1:
                request = _a.sent();
                console.log('request var', request);
                return [4 /*yield*/, renderTrucksAsAdmin()];
            case 2:
                _a.sent();
                return [3 /*break*/, 4];
            case 3:
                er_1 = _a.sent();
                console.error(er_1);
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); };
// Delete
var handleDelete = function (id) { return __awaiter(_this, void 0, void 0, function () {
    var error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                console.log(id);
                _a.label = 1;
            case 1:
                _a.trys.push([1, 4, , 5]);
                if (!id)
                    return [2 /*return*/, alert('Id not found!')];
                return [4 /*yield*/, axios["delete"]("/trucks/" + id)];
            case 2:
                _a.sent();
                return [4 /*yield*/, renderTrucksAsAdmin()];
            case 3:
                _a.sent();
                return [3 /*break*/, 5];
            case 4:
                error_2 = _a.sent();
                console.error(error_2);
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); };
var handleEdit = function (id) {
    var editModal = document.querySelector('.edit-modal-wrapper');
    editModal.classList.remove('hide');
    var idToEdit = document.querySelector('#id-edit');
    idToEdit.value = id;
};
var handleEditSubmit = function (event) { return __awaiter(_this, void 0, void 0, function () {
    var type, price, image, id, res;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                event.preventDefault();
                type = event.target.elements['type-edit'].value;
                price = event.target.elements['price-edit'].value;
                image = event.target.elements['image-edit'].value;
                id = event.target.elements['id-edit'].value;
                return [4 /*yield*/, axios.put("/trucks/" + id, { type: type, price: price, image: image })];
            case 1:
                res = _a.sent();
                console.log(res);
                return [4 /*yield*/, renderTrucksAsAdmin()];
            case 2:
                _a.sent();
                return [2 /*return*/];
        }
    });
}); };
